#!/Users/qbaghi/anaconda3/bin/python
# -*- coding: utf-8 -*-
...
if __name__ == '__main__':
    ...
    t1 = time.time()
    af, ef, tf = signal_cls.compute_signal_freq(f_pos, params, del_t, tobs, channel='TDIAET', real_imag=False, ldc=False)
    t2 = time.time()
    print("LISABeta time: " + str(t2-t1))
    t3 = time.time()
    af2, ef2, tf2 = signal_cls.compute_signal_freq(f_pos, params, del_t, tobs, channel='TDIAET', real_imag=False, ldc=True)
    t4 = time.time()
    ...
    time_signal = [ifft(sf_sym * fs)[0:N] for sf_sym in freq_signal_sym]
    time_signal2 = [ifft(sf_sym * fs)[0:N] for sf_sym in freq_signal_sym2]
    ...
    a_noise = noise.generateNoiseFromDSP(np.sqrt(psd_ae), fs, myseed=np.int(1234))[0:N]
    e_noise = noise.generateNoiseFromDSP(np.sqrt(psd_ae), fs, myseed=np.int(5678))[0:N]
    t_noise = noise.generateNoiseFromDSP(np.sqrt(psd_t), fs, myseed=np.int(9101))[0:N]
    ...
    save_simulation(t_vect, [a_noise, e_noise, t_noise], time_signal,
                    ...
                    save_file_path='/Users/qbaghi/Codes/data/simulations/mbhb/simulation_3.hdf5')
    #
    # ===========================
    # Plots
    # ===========================
    ...
    fp.xlims = [24.7, 25.1]
    # fig1, ax1 = fp.plot([t_vect / day],
    #                     [time_signal[0]],
    #                     ['k'], [2], ['Signal alone'],
    #                     linestyles=['solid'], zorders=[1])
    # fig1.savefig('./lisabeta_waveform.pdf')
    ...
    fp2.ylabel = 'TDI A [fractional frequency]'
    fig2, ax2 = fp2.plot([freq], [np.abs(fft(a_meas)*2/N)], ['k'], [1], 'TDI X', linestyles=['solid'], zorders=[1])
    plt.show()
