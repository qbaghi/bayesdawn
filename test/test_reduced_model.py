...
if __name__ == '__main__':
    ...
    from bayesdawn.bayesdawn import datamodel
    ...
    signal_only = [datamodel.TimeSeries(signal.astype(np.float), del_t=del_t) for signal in signal_list]
    ...
    t1 = time.time()
    a_list = signal_cls.design_matrix_freq(f_band, params_intr,  del_t, 0, tobs, complex=True)
    ...
    # # Plots
    # plt.figure(0)
    # plt.loglog(f_band, np.abs(signal_only_fft[0][i_band]), 'k', linewidth=2, label='from data')
    # plt.loglog(f_band, np.abs(signal_exact[0]), 'b--', label='from waveform')
    # plt.loglog(f_band, np.abs(signal_est), 'r:', label='recovered')
    # plt.legend()
    #
    # plt.figure(1)
    # plt.semilogx(f_band, np.angle(signal_only_fft[0][i_band]), 'k', linewidth=2, label='from data')
    # plt.semilogx(f_band, np.angle(signal_exact[0]), 'b--', label='from waveform')
    # plt.semilogx(f_band, np.angle(signal_est), 'r:', label='recovered')
    # plt.legend()
