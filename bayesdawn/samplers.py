...
def clipcov(X, nit=3, n_sig=5):
    ...
    for i in range(nit):
        ...
        inds_tuple = [
            np.where(np.abs(dX[:, k]) <= n_sig * np.sqrt(cov[k, k]))[0]
...
class MHSampler(object):
    ...
    def sample(self, x, logp_x):
        ...
        x_prime = x + self.L_func(
            np.random.normal(loc=0.0, scale=1.0, size=self.N_params)
        )
    ...
    def run_mcmc(self, x0, cov0, Ns, verbose=False, cov_update=100):
        ...
        if not verbose:
            for s in range(1, Ns):
                x_samples[s, :], logp_samples[s] = self.sample(
                    x_samples[s - 1, :], logp_samples[s - 1]
                )
        else:
            for s in range(1, Ns):
                x_samples[s, :], logp_samples[s] = self.sample(
                ...
                if s % cov_update == 0:
                    ...
                    # formula:
                    self.set_cov(
                        np.cov(self.x_samples[0 : s + 1, :].T) * 2.38**2 / self.N_params
                    )
...
class ExtendedPTMCMC(ptemcee.Sampler):
    ...
    def single_sample(self, it, n_it, n_update, n_thin, callback):
        self.position, lnlike0, lnprob0 = self.sample(
            self.position, n_it, thin=n_thin, storechain=True
        )
    ...
    def run(
        ...
        for pos, lnlike0, lnprob0 in self.sample(
            pos, n_it, thin=n_thin, storechain=True
        ):
            if ((it % n_save == 0) & (it != 0)) | (it == n_it - 1):
                print("Save data at iteration " + str(it) + "...")
                file_object = open(save_path + "chain.p", "wb")
                pickle.dump(
                    self.chain[:, :, 0 : it // n_thin, :], file_object, protocol=4
                )
                file_object.close()
                file_object = open(save_path + "lnprob.p", "wb")
                pickle.dump(
                    self.logprobability[:, :, 0 : it // n_thin], file_object, protocol=4
                )
                file_object.close()
                print("Data saved.")
...
if dynesty_on:
        ...
        def run(
            ...
            for it, res in enumerate(self.sample(maxiter=n_it)):
                ...
                if (it % n_save == 0) & (it != 0):
                    print("Save data at iteration " + str(it))
                    df = pd.DataFrame(
                        self.results.samples[-n_save:, :], columns=param_names
                    )
    ...
    def run_and_save(
        ...
        if dynamic:
            ...
            while True:
                # evaluate stop
                stop = stopping_function(sampler.results, stop_kwargs={"pfrac": 1.0})
                if not stop:
                    # derive bounds
                    logl_bounds = weight_function(
                        sampler.results, wt_kwargs={"pfrac": 1.0}
                    )
                    for results in sampler.sample_batch(
                        logl_bounds=logl_bounds, maxiter=n_iter
                    ):
                        it += 1
                        # If it is a multiple of n_save, save data
                        if it % n_save == 0:
