...
def periodogram_mean(func, fe, n_data, f_zero=None):
    ...
    power = np.int(np.log(n_data) / np.log(2.0)) + 4
    ...
    X = R[0:n_data] * (1.0 - np.abs(n) / np.float(n_data))

    return 1.0 / fe * (fft(X) + n_data * ifft(X) - R[0]), R[0:n_data]


def periodogram_mean_masked(
    func, fe, n_data, n_freq, mask, n_points=None, n_conv=None, normal=True
):
    """
    ...
    if n_points is None:
        # 1. Calculation of the autocovariance function Rn
        power = np.int(np.log(2 * n_data) / np.log(2.0))  # + 1
        # Number of points for the integration
        n_points = 2**power
    ...
    Pm = 1.0 / fe * (fft(X, n_freq) + n_freq * ifft(X, n_freq) - R[0] * lambda_N[0])
