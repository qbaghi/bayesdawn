...
class GWModel(object):
    ...
    def __init__(
        ...
        if len(channels) == 1:
            ...
            self.concatenate_data_pos = lambda x_list: np.concatenate(
                [x[self.inds_pos] for x in x_list]
            )
    ...
    def matrix_model(self, params):
        ...
        return self.smodel.design_matrix_freq(
            self.f_pos, params, self.ts, self.tobs, channel=self.channels, complex=True
        )

    def compute_frequency_signal(self, params):
        y_gw_fft = np.zeros(self.N, dtype=np.complex128)

        y_gw_fft[self.inds_pos] = self.smodel.compute_signal_freq(
    ...
    def log_likelihood(self, params, spectrum, y_fft):
        ...
        if not self.reduced:
            ...
            s_fft_stack = self.concatenate_model(
                self.smodel.compute_signal_freq(
                    self.f_pos,
                    params,
                    self.ts,
                    self.tobs,
        ...
        # For a reduced likelihood model
            ...
            amplitudes = [
                gls(mat_list[i], spectrum[i][self.inds_pos], y_fft[i][self.inds_pos])
    ...
    def compute_matrices(self, params, s):
        ...
        if self.nsources == 1:
            ...
            mat_freq = np.hstack(
                [
                    self.matrix_model(params[i * self.ndim : (i + 1) * self.ndim])
    ...
    def ls_estimate_beta(self, ZI, ApS, data_fft):
        ...
        data_real = np.concatenate(
            (data_fft[self.inds_pos].real, data_fft[self.inds_pos].imag)
        )
