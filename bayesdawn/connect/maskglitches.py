...
def BH_lowpass(data, t_win=100, t_sam=5, fs=10):
    ...
    assert np.isclose(t_sam * fs, int(t_sam * fs), rtol=1e-5), (
        "Downsampling time must be multiple of sampling time."
    )
    assert np.isclose(t_win * fs, int(t_win * fs), rtol=1e-5), (
        "Windowing time must be multiple of sampling time."
    )
    assert np.isclose(step_win / step_sam, int(step_win / step_sam), rtol=1e-5), (
        "Watch out, t_win must be multiple of t_sam."
    )

    dtarr = np.diff(data["t"])
    assert np.isclose(dtarr[0], dt, rtol=1e-5), (
        "Aaargh, sampling frequency is not consistent with data."
    )  # just check fs
    assert np.allclose(dtarr, dt, rtol=1e-5), (
        "Aaargh, your data are not equally sampled in time."
    )  # just check sampling time
    ...
    outts = [
        np.convolve(data["t"], onearray, mode="valid")
    ]  # just a simple way to get times, computationally more expensive than linspace, but safer
    for tdi in names:
...
def detect_glitch_outliers(data, plot=True, threshold=10):
    ...
    peaks, properties = scipy.signal.find_peaks(
        np.abs(data[n]), height=threshold * mad, threshold=None, distance=1
    )
        ...
        for k in data_mask.keys():
            for tdi in data_mask[k].dtype.names[1:]:
                for pk in peaks[: 2 * glitchnum]:
                    if longglitch:
                        data_mask[k][tdi][
                            pk - n // 50 * glitchlen : pk + n * glitchlen
                        ] = 0.0
                    else:
