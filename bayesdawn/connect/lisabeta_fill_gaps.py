...
def view_gaps(ts, ys, maskinfo, nwing=20, labels=None):
    ...
    fig, axs = plt.subplots(
        nchan * 2, n, figsize=[6.4 * n, 4.8 * nchan * 2], squeeze=False
    )
    for i in range(n):
...
def plot_compare_spectra(
    ...
    for ich in range(nchan):
        if LISAnoise is not None:
            noises += [
                pyLISAnoise.LISANoisePSDFunction(
                    LISAnoise, ich + 1, TDIrescaled=resc, TDI=TDItype
                ).apply(fs)
            ]
    ...
    for ich in range(nchan):
        ...
        largesets = [
            pdata[i] for i in range(len(pdata)) if stds[i] / stds.mean() > 0.001
        ]
        ax = axs[1, ich]
        for dataset in largesets:
            ...
            ax.hist(
                dataset[ich][1:-1].real * scalefac / np.sqrt(noises[ich][1:-1]),
                bins=nbins,
                density=True,
            )
        ...
        ax.legend(
            labels=[
                labels[i] for i in range(len(pdata)) if stds[i] / stds.mean() > 0.001
            ]
        )
        ...
        for dataset in largesets:
            ax.hist(
                dataset[ich][1:-1].imag * scalefac / np.sqrt(noises[ich][1:-1]),
                bins=nbins,
                density=True,
            )
...
class bdPSDmodel(psdmodel.PSD):
    ...
    def psd_fn(self, x):
        ...
        xx = x
        psd = pyLISAnoise.LISANoisePSDFunction(
            self.LISAnoise, self.chan, TDIrescaled=self.TDIrescaled
        ).apply(xx)
        ...
        if self.dered_f0 > 0:
            ...
            psd[xx < cut] *= (
                xx[xx < cut] / cut
            ) ** self.dered_pow  # hack the low-f fall-off test
        return psd
...
def splinePSDs_from_time_data(tdataset, fsamp):
    ...
    for ichan in range(n_chan):
        psd_cls = psdmodel.PSDSpline(
            n_data, fsamp, n_knots=20, d=3, fmin=fmin, fmax=fmax, ext=0
        )
...
def create_imputation(gapinfo, psd, nchan, method=None):
    ...
    for i in range(nchan):
        ...
        if method is not None and method == "woodbury":
            imp += [
                datamodel.GaussianStationaryProcess(s, mask, psd[i], method="woodbury")
            ]
        elif method is None:
            imp += [datamodel.GaussianStationaryProcess(s, mask, psd[i], na=60, nb=60)]
        else:
            raise ValueError("Not recognized: method=" + method)
...
def update_imputation(
    gapinfo, imp, nchan, resid_data, nfuzz=0, psd=None, verbose=False
):
    """
    ...
    if verbose:
        view_gaps(ts, [y, y_rec], gapinfo, labels=["orig", "new"])
    ...
    if verbose and psd is not None:
        ...
        plot_compare_spectra(
            [resid_data, resid, control, result],
            PSDset=psd,
            labels=["orig", "fuzzed", "control", "result"],
        )
    return result
    ...
    def ML_update(self, FD_noise_data, savefilebase=None):
        ...
        for ich in range(len(self.channels)):
            if self.have_real_data:
                datadict[self.channels[ich]] = (
                    FD_noise_data[:, 2 * ich + 1] + 1j * FD_noise_data[:, 2 * ich + 2]
                )
            else:
...
class FDimputation:
    def __init__(
        ...
        if self.verbose:
            print(
                "constructing FD imputation with",
                len(gap_intervals),
                "gaps\n gap_intervals=",
                gap_intervals,
            )
        ...
        if method == "woodbury":
            ...
            raise ValueError('Did not recognize method="' + method + '"')
        self.impargs = args
        if intergap_min is None:
            intergap_min = nab
    ...
    def compute_imps(self):
        ...
        s = np.zeros(len(mask))  # for residual 'signal' is zero
        imps = [
            datamodel.GaussianStationaryProcess(s, mask, psd, **self.impargs)
            for psd in self.PSDmodel.PSDs
        ]
    ...
    def compute_gap_info(self, fs):
        ...
        if self.verbose:
            print(
                "computing gap info for",
                len(gap_intervals),
                "gaps\n initial gap_intervals =",
                gap_intervals,
            )

        if not len(gap_intervals.shape) == 2 or not gap_intervals.shape[1] == 2:
            raise ValueError(
                "obs_params.gap_intervals should provide a list of pairs [[tstart,tend],[...]] indicating the temporal location of data gaps."
            )
    ...
    def apply_imputation(
        self, resid_data, psd=None, complex_data=False, report_mean_squares=False
    ):
        """
        Function to apply a set of imputation models to  multi-channel Fourier-domain residual data.
        ...
        if report_mean_squares:
            ...
            print(
                "masked,unmasked mean-sq and ratio:",
                maskedmeansq,
                unmaskedmeansq,
                maskedmeansq / unmaskedmeansq,
            )
