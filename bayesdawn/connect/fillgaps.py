...
def load_tdi_timeseries(
    ...
    for comb in tdi["noisefree"].dtype.names[1:]:  # loop on the TDI combination names
        ...
        tdi["glitch"][comb] = (
            tdi["noisefree"][comb] - tdi["sky"][comb] - tdi["gal"][comb]
        )
    ...
    if generate_additional_datasets:
        ...
        for ds, dsgap in zip(
            ["clean", "noise", "sky", "noiseglitch"], additional_datasets
        ):
...
def find_gaps(times, values, identifier=np.isnan):
    ...
    gapsstart = (
        np.hstack([np.array([gaps[0]]), (gaps[1:])[gaps[1:] - gaps[:-1] > dt]]) - dt / 2
    )
    # same thing with endpoints, but reversed
    gapsend = (
        np.hstack([(gaps[:-1])[gaps[1:] - gaps[:-1] > dt], np.array([gaps[-1]])])
        + dt / 2
    )
...
def build_orthogonal_tdi(tdi_xyz, skip=100):
    ...
    if type(tdi_xyz) is dict:
        ...
        for k in tdi_xyz.keys():
            ...
            T = (
                tdi_xyz[k]["X"][skip:] + tdi_xyz[k]["Y"][skip:] + tdi_xyz[k]["Z"][skip:]
            ) * float(1.0 / np.sqrt(3))

            data[k] = np.rec.fromarrays(
                [tdi_xyz[k]["t"][skip:], A, E, T], names=["t", "A", "E", "T"]
            )
        ...
        E = (
            tdi_xyz["X"][skip:] - 2 * tdi_xyz["Y"][skip:] + tdi_xyz["Z"][skip:]
        ) / np.sqrt(6)
        T = (tdi_xyz["X"][skip:] + tdi_xyz["Y"][skip:] + tdi_xyz["Z"][skip:]) * float(
            1.0 / np.sqrt(3)
        )

        data = np.rec.fromarrays(
            [tdi_xyz["t"][skip:], A, E, T], names=["t", "A", "E", "T"]
        )
...
def fft_olap_psd(
    ...
    if fs is None:
        if data_array.dtype.names:
            ...
            raise ValueError(
                "Specify the sampling frequency of data using keyword fs, e.g. fs = 0.01 "
            )
    if chan is None:
        if not data_array.dtype.names:
            data = data_array
            ...
            raise ValueError(
                "Specify a channel name using keyword chan, e.g. chan = 'A' "
            )
    elif type(chan) is str:
        data = data_array[chan]
    ...
    if n == 1 or segment_size >= ndata:
        ...
        x_seg = [
            ...
            for i in range(0, len(data) - segment_size, segment_size - overlap_size)
        ]
    ...
    for i in range(total_segments):
        ...
        fft_segment[i] = np.fft.fft(
            windowed_segment, segment_size
        )  # fft automatically pads if n<nfft
        # sum over segments
    ...
    scalefac = (
        2
        * powerDensity_averaging
        * powerDensity_normalization
        * powerDensity_transformation
    )
...
def generate_freq_data(data, split=False, fmin=1e-6, win="hanning"):
    ...
    if split:
        ...
        for n in range(7):
            ...
            f, psd, fft, fftscalefac = fft_olap_psd(
                data, chan=tdi[n], nperseg=fs / fmin, win=win
            )
            if n == 0:
                fdata[n] = f
                psddata[n] = f
        ...
        fdata = np.recarray(
            shape=(np.int32(fs / fmin / 2),),
            dtype={
                "names": ("f",) + names,
                "formats": [np.float64] + 3 * [np.complex128],
            },
        )
        psddata = np.recarray(
            shape=(np.int32(fs / fmin / 2),),
        ...
        for tdi in names:
            ...
            f, psd, fft, fftscalefac = fft_olap_psd(
                data, chan=tdi, nperseg=fs / fmin, win=win
            )
            fdata[tdi] = fft
            psddata[tdi] = psd
        fdata["f"] = f
...
def pack_FD_data(array, channels=["A", "E", "T"]):
    ...
    data = np.recarray(
        ...
        dtype={
            "names": ["f"] + names,
            "formats": [np.float64] + nchan * [np.complex128],
        },
    )
    data["f"] = array[:, 0]
...
def plot_compare_spectra_timeseries(
    data,
    noise_model="spritz",
    freq_bands=None,
    fmin=1e-6,
    fmax=2e-2,
    tdi_vars="orthogonal",
    labels=["Signal + Noise", "Noise", "Signal"],
    save=False,
    fname=None,
):
    """
    ...
    # create figure
    fig = plt.figure(
        figsize=[2 * nchan * nfbands, 3 * nchan], constrained_layout=True, dpi=120
    )
    ...
    for row, subrow in enumerate(subfigs):
        ...
        for i, a in enumerate(ax):
            ...
            for idx, d in enumerate(data):
                _, _, fft, fft_scalefac = fft_olap_psd(
                    d, chan=names[row], nperseg=1 / dt / fmin
                )
                # set up scale factor for fft
                scalefac = np.sqrt(2 * fft_scalefac)
                a.hist(
                    fft[fband].real * scalefac / np.sqrt(S[fband]),
                    bins=nbins,
                    density=True,
            ...
            a.set_xlim([-6, 6])
            a.set_title(
                "{:0.1f}-{:0.1f} mHz".format((flim[i][0] * 1e3), (flim[i][1] * 1e3))
            )
        ...
        for i, a in enumerate(ax):
            ...
            for idx, d in enumerate(data):
                ...
                scalefac = np.sqrt(2 * fft_scalefac)
                a.hist(
                    fft[fband].imag * scalefac / np.sqrt(S[fband]),
                    bins=nbins,
                    density=True,
                    label=data_labels[idx],
                )
...
def get_ldc_gap_mask(data, mode):
    ...
    # the difference between two subsequent samples is more than dt
    gapstarts = np.hstack(
        [np.array([gaps_data[0]]), (gaps_data[1:])[gaps_data[1:] - gaps_data[:-1] > dt]]
    )
    # same thing with endpoints, but reversed
    gapends = np.hstack(
        [
            (gaps_data[:-1])[gaps_data[1:] - gaps_data[:-1] > dt],
            np.array([gaps_data[-1]]),
        ]
    )
    if mode == "times":
...
def construct_gap_mask(n_data, n_gaps=30, gap_length=10, verbose=False, seed=None):
    ...
    if verbose:
        ...
        for k in range(n_gaps):
            print(
                "  gap" + str(k), "(" + str(gapstarts[k]) + ":" + str(gapends[k]) + ")"
            )
...
def view_gaps(
    ...
    for chan, subfig in enumerate(subfigs):
        ...
        for i in range(ngap):
            ...
            ax = axs[0][i]
            ax.set_prop_cycle(
                cycler(color=["tab:blue", "tab:orange"], linestyle=["-", ":"])
            )
            l = 0
            for yi in ys:
                ...
                l += 1
            ax.plot(
                ts[i0:iend], yg[chan][i0:iend], label="gapped", color="tab:cyan", ls="-"
            )
            ...
            ax = axs[1][i]
            ax.set_prop_cycle(
                cycler(color=["tab:purple", "tab:olive"], linestyle=["-", ":"])
            )
            std = []
            for yi in ys:
            ...
            if labels is not None:
                ax.legend(labels=[l + " - gapped " for l in labels])
            ax.grid()
            ax.set_xlabel("Time [s]")
            ax.set_ylabel("Amplitude")
            ...
            if histogram:
                for j in range(2, 4):
                    ...
                    for l, yi in enumerate(ys):
                        f, _, fft, fft_scalefac = fft_olap_psd(
                            yi[chan][i0:iend],
                            fs=fs,
                            nperseg=len(yi[chan][i0:iend]) / 3,
                            win="taper",
                        )
                        # assess number of bins from noise data
                        ...
                        Nmodel = get_noise_model(
                            noise_model,
                            f,
                            wd=0,
                            orbits=orbits,
                            t_obs=len(yi[chan]) / fs,
                        )
                        S = Nmodel.psd(
                            tdi2=True, option=channels[chan], freq=f, equal_arms=False
                        )
                        if j % 2:
                            plotvals = fft.imag * scalefac / np.sqrt(S)
                            ax.set_xlabel("Imag part deviation - whitened data")
                            ax.hist(
                                plotvals, bins=nbins, density=True, label=labels[l]
                            )  # +' '+str(int(min(plotvals)))+'<x<'+str(int(max(plotvals))))
                            ax.set_xlabel("Imag part deviation - whitened data")
                            ax.hist(
                                fft.imag * scalefac / np.sqrt(S),
                                bins=nbins,
                                density=True,
                                label=labels[l],
                            )
    ...
    if save:
        if fname is not None:
            fig.savefig(
                fname + "_gaps.png", dpi=120, bbox_inches="tight", facecolor="white"
            )
            return ratio
...
def create_imputation(data, channel, mask, noise_model="spritz"):
    ...
    imp_cls = datamodel.GaussianStationaryProcess(
        s, mask, psd_cls, method="nearest", na=100, nb=100
    )
...
def update_imputation(
    data_rec, imp_cls, channel, fit_type="log_spline", fit_dof=15, fmin=7e-6
):
    """
    Update imputation
    ...
    psdmod = psdmodel.ModelFDDataPSD(
        data=fd,
        channel=channel,
        fit_type=fit_type,
        fit_dof=fit_dof,
