...
def postprocess(chain, lnprob, names, par0, n_burn=500, n_thin=1, n_bins=40, k=4):
    ...
    limits = [
        [medians[i] - k * stds[i], medians[i] + k * stds[i]]
    ...
    fig, axes = resanalysis.cornerplot(
        [chain_flatten],
        par0,
        offset,
        scales,
...
def get_simu_parameters(config_path, simu_path=None, ldc=True, intrinsic=True):
    ...
    if sampler_type == "ptemcee":
        ...
        try:
            chain0 = loadings.load_samples(
                os.path.dirname(config_path) + "/" + prefix + "_final_save.p"
            )
        except ValueError:
            print("No final dinesty result, loading the initial file.")
            chain0 = loadings.load_samples(
                os.path.dirname(config_path) + "/" + prefix + "_initial_save.p"
            )
