...
if __name__ == "__main__":
    ...
    if maskname == "random":
        ...
        maskfile = (
            "/Users/qbaghi/Codes/data/masks/2018-10-29_15h22-12_mask_antenna.hdf5"
        )
        if np.abs(truths[2] - 1e-4) < 1e-8:
            n_wind = 1500
        elif np.abs(truths[2] - 2e-4) < 1e-8:
    ...
    maskfh5 = h5py.File(
        "/Users/qbaghi/Codes/data/masks/2018-11-14_15h48-51_mask_micrometeorites.hdf5",
        "r",
    )
    M2 = maskfh5["mask"][()]
    maskfh5.close()
    ...
    chain_psd = resanalysis.load_chain(
        [filepaths[j0] + noisenames[j0]], data_name="psd/samples"
    )
    logp_psd = resanalysis.load_chain(
        [filepaths[j0] + noisenames[j0]], data_name="psd/logpvals"
    )
    fc, S_samples, S_map, S_map_low, S_map_up = psdplot.compute_psd_map(
        chain_psd, logp_psd, fs, N
    )
    ...
    # Draw the signal
    y_gw_fft = gwm.draw_frequency_signal(
        param_map * rescale, S, fft(y_wind) * N / np.sum(M)
    )
    ...
    y_mis_samples = np.array(
        [
            imp.single_imputation(y_seg - s_gw, M_binary[i1:i2], C, S2) + s_gw[ind_misj]
            for k in range(1000)
        ]
    )
    ...
    Y2 = list(
        [
            y[i1:i2] * conf.scale,
            M_binary[i1:i2] * 2e-19 - 3e-20,
            y_rec_m[i1:i2] * conf.scale,
        ]
    )
    ...
    Xin = [
        t[i1:i2][inds] / day,
        t[i1:i2][ind_misj][inds2] / day,
        t[i1:i2][inds] / day,
        t[i1:i2][ind_misj][inds2] / day,
    ]
    Yin = [
        y_seg[inds] * conf.scale,
        y_mis_samples[0][inds2] * conf.scale,
        dTDI_gw[i1:i2, 1][inds],
    ...
    axins2 = inset_axes(
        ax2, 3, 2, loc="upper center", axes_kwargs={"xlim": [x1, x2], "ylim": [y1, y2]}
    )
    ...
    plt.vlines(
        x=t[i1:i2][ind_misj[0]] / day, ymin=ymin, ymax=ymax, linewidth=1, color="r"
    )

    fig2.savefig(
        "/Users/qbaghi/Documents/articles/papers/papers/gaps/figures/time_series/time_series_gap_rec_inset.pdf"
    )
    # plt.draw()
    plt.show()
