...
def lisabeta_waveform(
    ...
    wftdi = lisa.GenerateLISATDI_SMBH(
        params_dic,
        minf=minf,
        maxf=maxf,
        tmin=tmin,
    ...
    return [
        shiftTime(freq, sig["ch" + str(i)], tshift).conj() * scale for i in channels
    ]


def compute_signal(
    p_sampl, freq, minf=1e-5, maxf=0.1, t_offset=0.0, channels=[1, 2], scale=1.0
):
...
def compute_signal_reduced(
    ...
    # Compute amplitudes
    amps = [
        matrixalgebra.gls(data_dft[i], mat_list[i], sn[i]) for i in range(len(channels))
    ]
...
if __name__ == "__main__":
    ...
    # =========================================================================
    parser = OptionParser(
        usage="usage: %prog [options] YYY.txt", version="10.28.2018, Quentin Baghi"
    )
    ...
    if "LDC" in config["InputData"]["FilePath"]:
        ...
        tvect, signal_list, noise_list, params, tstart, del_t, tobs = (
            loadings.load_simulation(config["InputData"]["FilePath"])
        )
        # Form data = signal + noise
        data_list = [signal_list[i] + noise_list[i] for i in range(len(signal_list))]
        ...
        if dec:
            ...
            data_pre_list = [preprocess.filter(dat, fc, del_t) for dat in data_list]
        else:
    ...
    print(
        "Ideal decay number: "
        + str(
            np.int((config["InputData"].getfloat("EndTime") - p_sampl[2]) / (2 * del_t))
        )
    )
    ...
    if (include_gaps) & (not imputation) & frequency_windowing:
        f1, f2 = physics.find_distorted_interval(
            mask, p_sampl, tstart, del_t, margin=0.5
        )
        f1 = np.max([f1, 0])
        ...
        inds = np.where((f_minimum <= freq_d) & (freq_d <= f_maximum) & (freq_d >= f2))[
            0
        ]
    else:
    ...
    if psd_estimation | (psd_model == "spline"):
        ...
        psd_cls = [
            psdmodel.PSDTheoretical(
                tm.shape[0], 1 / del_t, ch, scale=scale, fmin=None, fmax=None
            )
            for ch in ["A", "E"]
        ]
        sn = [psd.calculate(freq_d[inds]) for psd in psd_cls]
    ...
    t1 = time.time()
    if reduced:
        aft, eft = ll_cls.compute_signal_reduced(
            p_sampl[i_sampl_intr], ll_cls.data_dft, sn
        )
    else:
    ...
    bounds = [
        [
            config["ParametersLowerBounds"].getfloat(name),
    ...
    if initialization == "prior":
        pos0 = np.random.uniform(
            lower_bounds, upper_bounds, size=(ntemps, nwalkers, len(upper_bounds))
        )
    ...
    if (not psd_estimation) & (not imputation):
        sampler = samplers.ExtendedPTMCMC(
            nwalkers,
            len(names),
            log_likelihood,
            posteriormodel.logp,
        ...
        t1 = time.time()
        result = sampler.run(
            int(config["Sampler"]["MaximumIterationNumber"]),
        ...
        t1 = time.time()
        aux0 = np.full((nwalkers, par_aux0.shape[0]), par_aux0, dtype=np.complex128)
        result = sampler.run(
            n_iter,
            n_save,
            n_thin,
